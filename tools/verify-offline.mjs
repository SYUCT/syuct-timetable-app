...
const bt=path.join(sdk,'build-tools/35.0.0'),android=path.join(sdk,'platforms/android-36/android.jar');
...
run(path.join(bt,'aapt2'),['link','--auto-add-overlay','-I',android,'--manifest',path.join(out,'AndroidManifest.xml'),'-R',path.join(out,'res.zip'),'-A',path.join(root,'app/src/main/assets'),'--java',path.join(out,'generated'),'--min-sdk-version','26','--target-sdk-version','35','--version-code','11','--version-name','0.2.9-alpha1','-o',path.join(out,'resources.apk')]);
