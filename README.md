...
当前版本 **0.2.19-alpha1**：普通通知展开后按开课时间、授课教师、课程地点分行；岛展开详情优先显示教室，再分行显示时间和教师，不再用分隔点拼接。厂商岛实际效果仍由手机系统决定，线上版本以 Releases 为准。详见 [UPDATE-0.2.19.md](UPDATE-0.2.19.md)。

## 使用

1. 从本仓库 Releases 下载已发布的 APK，在安卓手机安装。本次本地构建包名为 `SYUCT-Timetable-0.2.19-alpha1.apk`。设置中可点击「检测更新」。
...
双击项目中的 **`预览化大课表.command`**：自动启动已配置模拟器、覆盖安装 `artifacts/SYUCT-Timetable-0.2.19-alpha1.apk` 并打开 App。也可在终端运行 `zsh ./预览化大课表.command`。该入口适用于这台已配置的 Mac；其他电脑需先安装 SDK 并创建同名设备。
