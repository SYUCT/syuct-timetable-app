...
当前开发版本 **0.2.10-alpha1**。新增可选“准时提醒”，由用户授权系统“闹钟和提醒”后使用精确定时，不添加时钟闹钟，不常驻后台。未授权保留普通提醒。实时倒计时仍为默认关闭的试验功能，岛的样式由手机系统决定。见 [UPDATE-0.2.10.md](UPDATE-0.2.10.md)，已发布版本以 Releases 为准。

## 使用

1. 从本仓库 Releases 下载已发布的 APK，在安卓手机安装。本次本地构建包名为 `SYUCT-Timetable-0.2.10-alpha1.apk`。设置中可点击「检测更新」。
...
双击项目中的 **`预览化大课表.command`**：自动启动已配置模拟器、覆盖安装 `artifacts/SYUCT-Timetable-0.2.10-alpha1.apk` 并打开 App。也可在终端运行 `zsh ./预览化大课表.command`。该入口适用于这台已配置的 Mac；其他电脑需先安装 SDK 并创建同名设备。
