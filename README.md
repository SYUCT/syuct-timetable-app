...
当前开发版本 **0.2.11-alpha1**。小米增加“左侧彩色校徽＋课程前三字、右侧开课时间”的官方模板参数；vivo/iQOO 保留原子岛实验参数。系统小图标仍可能染色；厂商模板实际效果和资格待真机确认，不承诺必定上岛。见 [UPDATE-0.2.11.md](UPDATE-0.2.11.md)，已发布版本以 Releases 为准。准时提醒仍为可选授权，不常驻后台。

## 使用

1. 从本仓库 Releases 下载已发布的 APK，在安卓手机安装。本次本地构建包名为 `SYUCT-Timetable-0.2.11-alpha1.apk`。设置中可点击「检测更新」。
...
双击项目中的 **`预览化大课表.command`**：自动启动已配置模拟器、覆盖安装 `artifacts/SYUCT-Timetable-0.2.11-alpha1.apk` 并打开 App。也可在终端运行 `zsh ./预览化大课表.command`。该入口适用于这台已配置的 Mac；其他电脑需先安装 SDK 并创建同名设备。
