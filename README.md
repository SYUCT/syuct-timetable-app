...
当前版本 **0.2.13-alpha1**。恢复小米通用实时通知，不再根据焦点权限自动选择未验证的专用模板。预览按钮增加即时反馈、后台发送及系统接收确认。厂商视觉效果仍待真机确认，见 [UPDATE-0.2.13.md](UPDATE-0.2.13.md)。已发布版本以 Releases 为准；不新增权限或常驻服务。

## 使用

1. 从本仓库 Releases 下载已发布的 APK，在安卓手机安装。本次本地构建包名为 `SYUCT-Timetable-0.2.13-alpha1.apk`。设置中可点击「检测更新」。
...
双击项目中的 **`预览化大课表.command`**：自动启动已配置模拟器、覆盖安装 `artifacts/SYUCT-Timetable-0.2.13-alpha1.apk` 并打开 App。也可在终端运行 `zsh ./预览化大课表.command`。该入口适用于这台已配置的 Mac；其他电脑需先安装 SDK 并创建同名设备。
