...
当前代码版本 **0.2.4-alpha1**。设置页新增手动检测更新，小组件校徽放大。仅点击后访问官网，不弹窗、不自动下载、不新增权限。变更与验收记录见 [UPDATE-0.2.4.md](UPDATE-0.2.4.md)，已发布版本以 Releases 为准。

## 使用

1. 从本仓库 Releases 下载已发布的 APK，在安卓手机安装。0.2.4 安装包名为 `SYUCT-Timetable-0.2.4-alpha1.apk`。本版起可在设置中点击「检测更新」，已有旧版需先手动覆盖安装一次。
...
双击项目中的 **`预览化大课表.command`**：自动启动已配置模拟器、覆盖安装 `artifacts/SYUCT-Timetable-0.2.4-alpha1.apk` 并打开 App。也可在终端运行 `zsh ./预览化大课表.command`。该入口适用于这台已配置的 Mac；其他电脑需先安装 SDK 并创建同名设备。
