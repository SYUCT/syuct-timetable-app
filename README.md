...
当前版本 **0.2.0-alpha1**。新增横向七列全览、首页第一周日期弹窗、今日课表桌面小组件及当前课标记；修复安卓状态栏遮挡和全览固定列遮挡导航。变更与验收记录见 [UPDATE-0.2.0.md](UPDATE-0.2.0.md)。

## 使用

1. 从本仓库 Releases 下载 `SYUCT-Timetable-0.2.0-alpha1.apk`，在安卓手机安装。
...
小组件显示当前课并自动更新。新增每次上课前15分钟的系统通知，首次使用可开启，设置页可关闭或调整权限。使用手机默认通知声音，尊重静音和勿扰。准时提醒需要用户允许通知与闹钟权限；无权限或受系统省电限制时可能延迟。无常驻后台服务。详见 [REMINDERS.md](REMINDERS.md)。
...
双击项目中的 **`预览化大课表.command`**：自动启动已配置模拟器、覆盖安装 `artifacts/SYUCT-Timetable-0.2.0-alpha1.apk` 并打开 App。也可在终端运行 `zsh ./预览化大课表.command`。该入口适用于这台已配置的 Mac；其他电脑需先安装 SDK 并创建同名设备。
...
- 请求联网及开机完成广播权限，后者用于恢复小组件和上课提醒。提醒另申请通知及由用户控制的准时闹钟权限。不申请相机、通讯录、位置、外部存储，无常驻后台服务。
