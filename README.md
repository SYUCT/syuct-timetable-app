...
当前版本 **0.2.16-alpha1**：岛短文本改为课程名前四字，移除分钟刷新；系统卡片使用短摘要，增加完整课程详情入口。保留“闹钟和提醒权限”入口、课前15分钟触发、卡片系统计时和到时结束，不接入厂商 App ID，不新增权限或常驻服务。厂商岛实际效果仍待真机确认，线上版本以 Releases 为准。详见 [NOTICE-LAYOUT-REVIEW.md](NOTICE-LAYOUT-REVIEW.md) 和 [UPDATE-0.2.16.md](UPDATE-0.2.16.md)。

## 使用

1. 从本仓库 Releases 下载已发布的 APK，在安卓手机安装。本次本地构建包名为 `SYUCT-Timetable-0.2.16-alpha1.apk`。设置中可点击「检测更新」。
...
双击项目中的 **`预览化大课表.command`**：自动启动已配置模拟器、覆盖安装 `artifacts/SYUCT-Timetable-0.2.14-alpha1.apk` 并打开 App。也可在终端运行 `zsh ./预览化大课表.command`。该入口适用于这台已配置的 Mac；其他电脑需先安装 SDK 并创建同名设备。
...
- 请求联网及开机完成广播权限，后者用于恢复小组件和上课提醒。提醒需要通知权限；可选准时提醒另由用户授权 `SCHEDULE_EXACT_ALARM`。不申请相机、通讯录、位置、外部存储、悬浮窗权限，无常驻后台服务。
