...
当前版本 **0.2.20-alpha1**：普通通知把教室移到首行，时间和教师排在后面，避免收起时先显示教师却藏住教室；岛展开详情维持地点优先。厂商岛实际效果仍由手机系统决定，线上版本以 Releases 为准。详见 [UPDATE-0.2.20.md](UPDATE-0.2.20.md)。

## 使用

1. 从本仓库 Releases 下载已发布的 APK，在安卓手机安装。本次本地构建包名为 `SYUCT-Timetable-0.2.20-alpha1.apk`。设置中可点击「检测更新」。
...
双击项目中的 **`预览化大课表.command`**：自动启动已配置模拟器、覆盖安装 `artifacts/SYUCT-Timetable-0.2.20-alpha1.apk` 并打开 App。也可在终端运行 `zsh ./预览化大课表.command`。该入口适用于这台已配置的 Mac；其他电脑需先安装 SDK 并创建同名设备。
