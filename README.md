...
当前版本 **0.2.12-alpha1**。针对澎湃 OS 3 的反馈，补上焦点通知权限查询、分开原生模板与通用通知路径；通用短文本同时带课程简称和开课时间，小米图标改传原始彩色资源。厂商最终布局和图标染色仍待真机确认，不能把参数测试当作上岛验收。见 [UPDATE-0.2.12.md](UPDATE-0.2.12.md)，已发布版本以 Releases 为准。准时提醒仍为可选授权，不常驻后台。

## 使用

1. 从本仓库 Releases 下载已发布的 APK，在安卓手机安装。本次本地构建包名为 `SYUCT-Timetable-0.2.12-alpha1.apk`。设置中可点击「检测更新」。
...
双击项目中的 **`预览化大课表.command`**：自动启动已配置模拟器、覆盖安装 `artifacts/SYUCT-Timetable-0.2.12-alpha1.apk` 并打开 App。也可在终端运行 `zsh ./预览化大课表.command`。该入口适用于这台已配置的 Mac；其他电脑需先安装 SDK 并创建同名设备。
