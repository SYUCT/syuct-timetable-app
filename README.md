...
当前版本 **0.2.14-alpha1**。岛内改用系统15分钟倒计时，不再传课程名或开课时刻；展开按行显示课程名称、开课时间、教师与教室。预览同步为15分钟。不接入厂商 App ID。厂商视觉效果仍待真机确认，见 [UPDATE-0.2.14.md](UPDATE-0.2.14.md)。已发布版本以 Releases 为准；不新增权限或常驻服务。

## 使用

1. 从本仓库 Releases 下载已发布的 APK，在安卓手机安装。本次本地构建包名为 `SYUCT-Timetable-0.2.14-alpha1.apk`。设置中可点击「检测更新」。
