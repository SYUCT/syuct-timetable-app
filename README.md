...
当前版本 **0.2.15-alpha1**：固定显示“闹钟和提醒权限”入口；小米的岛短文本与普通标题使用剩余分钟数，展开保留完整课程信息。其他系统继续使用原生倒计时。不接入厂商 App ID，不新增权限或常驻服务。厂商岛实际效果仍待真机确认，线上版本以 Releases 为准。详见 [UPDATE-0.2.15.md](UPDATE-0.2.15.md) 和 [NOTICE-REVIEW.md](NOTICE-REVIEW.md)。

## 使用

1. 从本仓库 Releases 下载已发布的 APK，在安卓手机安装。本次本地构建包名为 `SYUCT-Timetable-0.2.15-alpha1.apk`。设置中可点击「检测更新」。
