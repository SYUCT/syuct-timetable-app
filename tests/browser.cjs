...
(async()=>{
 ...
 try{
  ...
  let updateDialogs=0;const trackUpdateDialog=()=>updateDialogs++;page.on('dialog',trackUpdateDialog);
  await page.locator('#checkUpdate').click();assert.equal(await page.locator('#checkUpdate').isDisabled(),true);assert.match(await page.locator('#updateStatus').innerText(),/正在连接/);assert.equal(await page.evaluate(()=>window.updateCalls),1);
  await page.evaluate(()=>window.receiveUpdateCheck({requestId:window.lastUpdateId,status:'current',versionCode:6,versionName:'0.2.4-alpha1'}));assert.equal(await page.locator('#updateStatus').innerText(),'当前已是最新版本。');assert.equal(await page.locator('#downloadUpdate').isVisible(),false);count++;console.log('PASS 手动检测中禁止重复点击，同版本不提供下载');
  const updateSaved=await page.evaluate(()=>localStorage.getItem('test-state'));
  await page.locator('#checkUpdate').click();await page.evaluate(()=>window.receiveUpdateCheck({requestId:window.lastUpdateId,status:'available',versionCode:7,versionName:'0.2.5-alpha1',notes:'修复说明\n<img src=x onerror=alert(1)>'}));
  assert.match(await page.locator('#updateStatus').innerText(),/0.2.5-alpha1/);assert.equal(await page.locator('#downloadUpdate').isVisible(),true);assert.equal(await page.locator('#updateNotes img').count(),0);assert.equal(await page.evaluate(()=>window.downloadedVersion),undefined);
  await page.locator('#downloadUpdate').click();assert.equal(await page.evaluate(()=>window.downloadedVersion),7);assert.equal(await page.evaluate(()=>localStorage.getItem('test-state')),updateSaved);
  await page.screenshot({path:path.join(__dirname,'../test-results/manual-update.png'),animations:'disabled'});count++;console.log('PASS 新版内联显示与纯文本说明，用户点击后才下载，不修改课表');
  await page.locator('#checkUpdate').click();const staleId=await page.evaluate(()=>window.lastUpdateId);assert.equal(await page.locator('#downloadUpdate').isVisible(),false);
  await page.clock.runFor(21000);assert.match(await page.locator('#updateStatus').innerText(),/超时/);assert.equal(await page.locator('#checkUpdate').isEnabled(),true);
